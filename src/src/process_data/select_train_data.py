...
def select_data(path_to_folder: str, path_to_train: str, num_per_domain: int) -> List:
    ...
    for domain in domains:
        ...
        if num_per_domain == 500:
            if not os.path.exists(f"{path_to_folder}/{num_per_domain}_per_domain/{domain}"):
                os.makedirs(f"{path_to_folder}/{num_per_domain}_per_domain/{domain}")
            
            with open(f"{path_to_folder}/{num_per_domain}_per_domain/{domain}/train.json", 'w') as f:
...
if __name__ == "__main__":
    ...
    for num_per_domain in [10, 30, 50, 100, 200, 500]:
        ...
        selected_data, all_data = select_data(path_to_folder, path_to_train, num_per_domain = num_per_domain)
        all_selected_data.extend(selected_data)
