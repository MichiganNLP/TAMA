...
def process() -> None:
    ...
    for k, v in all_tables.items():
        ...
        if row_headers != []:
            table_content = (
                "header"
                + " | "
                + " | ".join([" ".join(col_l) for col_l in col_headers])
            )
            ...
            if len(row_headers) > len(row_datas):
                table_content += " | [SEP] | ".join(
                    [
                        "".join(row_header)
                        for row_header in row_headers[len(row_datas) :]
                    ]
                )
    ...
    test_table_ids = len(set([ele["table_id"] for ele in test]))
    logger.info(f"Test tables: {test_table_ids}")
