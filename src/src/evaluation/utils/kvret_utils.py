""" 
From https://github.com/xlang-ai/UnifiedSKG/blob/main/metrics/kvret/response_entity_hit.py
for micro f1 score calculation
"""
# coding=utf-8
    ...
    for entity_item_a in [20, 30, 40, 50, 60, 70, 80, 90, 100]:
        for entity_item_b in [20, 30, 40, 50, 60, 70, 80, 90, 100]:
            utterance = utterance.replace(
                "{}-{}f".format(str(entity_item_a), str(entity_item_b)),
                "{}f-{}f".format(str(entity_item_a), str(entity_item_b)),
            )
    entities_in_this_utterance = []
    entities = sorted(
        list(set(list(global_entities.values()) + list(global_entities.keys()))),
        key=lambda i: len(i),
        reverse=True,
    )
...
def f1_score(y_pred, y_true, average="micro"):
    ...
    for y_true_item, y_pred_item in zip(y_true, y_pred):
        single_tp, single_fp, single_fn, single_f1, count = _compute_prf(
...
class Response_entity_hit(datasets.Metric):
    ...
    def _compute(
        ...
        entities_from_predictions_and_references = {
            "all": {"predictions_entities": [], "references_entities": []}
        }
        global_entities = kwargs["global_entities"]

        for prediction, response in zip(predictions, references):
            intents = response["intents"]
            prediction_entities = extract_entities_from_utterance(
                utterance=prediction, global_entities=global_entities
            )
            references_entities = extract_entities_from_utterance(
                utterance=response["response"], global_entities=global_entities
            )
            entities_from_predictions_and_references["all"][
                "predictions_entities"
            ].append(prediction_entities)
            entities_from_predictions_and_references["all"][
                "references_entities"
            ].append(references_entities)
            for intent in intents:
                if intent not in entities_from_predictions_and_references.keys():
                    entities_from_predictions_and_references[intent] = {
                        "predictions_entities": [],
                        "references_entities": [],
                    }
                entities_from_predictions_and_references[intent][
        ...
        for category in entities_from_predictions_and_references.keys():
            prediction_entities = entities_from_predictions_and_references[category][
                "predictions_entities"
            ]
            references_entities = entities_from_predictions_and_references[category][
                "references_entities"
            ]
            F1_scores["{}_micro".format(category)] = f1_score(
                y_pred=prediction_entities, y_true=references_entities, average="micro"
            )
            F1_scores["{}_macro".format(category)] = f1_score(
                y_pred=prediction_entities, y_true=references_entities, average="macro"
            )
