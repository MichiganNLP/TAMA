...
def eval() -> None:
    with open(f"{home_dir}/tablellm/datasets/instruction_following/processed/test.json", 'r') as f:
    ...
    for folder_name, ele in files.items():
        ...
        p = subprocess.run(["python", "-m", "src.evaluation.instruction_following_eval.evaluation_main", 
        ...
        for idx, line in enumerate(p.stdout.split("\n")):
            ...
            if idx == anchor_line_idx + 1: 
                ...
                instance_level_score = float(line.split("instruction-level: ")[-1]) * 100
        

        logger.info(folder_name, steps)
        logger.info(f"IFEval strict: prompt-level score: {prompt_level_score}, instance-level score: {instance_level_score}")

        print(f"{extract_info(folder_name)}, {steps}, {prompt_level_score}, {instance_level_score}")
