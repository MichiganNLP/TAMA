...
def evaluate() -> None:
    files = find_files(root_dir=root_dir, name="feverous_test")
        ...
        for line in data:
            ...
            if "original" in folder_name:
                ...
                if gold in predict and all(label not in predict for label in [x for x in possible_labels if x != gold]):
                    correct_num += 1
                elif gold == "refutes" and "refute" in predict and all(label not in predict for label in [x for x in possible_labels if x != gold]):
                    correct_num += 1
                elif gold == "supports" and "support" in predict and all(label not in predict for label in [x for x in possible_labels if x != gold]):
                    correct_num += 1
            else:
                if predict.startswith("refuted"):
                    predict = "refutes"
                elif predict.startswith("entailed"):
                    predict = "supports"
                
                if predict.startswith(gold):
                    correct_num += 1
        logger.info(folder_name, steps)
        logger.info(f"Feverous: {correct_num / len(data) * 100}")
