...
for i, learning_rate_filter in enumerate(unique_learning_rates):
    for j, epoch_filter in enumerate(unique_epochs):
        ...
        for data in [data_0, data_1, data_2, data_3, data_4, data_5]:
            ...
            filtered_df = df[
                (df["learning_rate"] == learning_rate_filter)
                & (df["epochs"] == epoch_filter)
            ]
        ...
        heatmap_data = filtered_df.pivot_table(
            values="performance", index="train_dataset", columns="test_dataset"
        )
...
fig.colorbar(
    plt.cm.ScalarMappable(
        cmap="YlGnBu", norm=plt.Normalize(vmin=performance_min, vmax=performance_max)
    ),
    ax=axes,
    location="right",
    shrink=0.6,
