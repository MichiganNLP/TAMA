...
for lr in df['learning_rate'].unique():
    ...
    plt.plot(subset['epochs'], subset['hitab_performance'], marker='o', markersize=MARKSIZE, label=f'HiTab')
    plt.plot(subset['epochs'], subset['fetaqa_performance'], marker='s', markersize=MARKSIZE, label=f'FeTaQA')
    plt.plot(subset['epochs'], subset['tabfact_performance'], marker='^', markersize=MARKSIZE, label=f'TabFact')
    plt.plot(subset['epochs'], subset['feverous_performance'], marker='v', markersize=MARKSIZE, label=f'FEVEROUS')
    plt.plot(subset['epochs'], subset['ifeval_performance'], marker='D', markersize=MARKSIZE, label=f'IFEval')
    plt.plot(subset['epochs'], subset['mmlu_performance'], marker='*', markersize=MARKSIZE, label=f'MMLU')

    # plt.xlabel('Steps')x f

    plot_details(df=df)
    # Display the plot
    plt.savefig(f"../figures/llama31_instruct_lr_{lr}.pdf")
    plt.close()
